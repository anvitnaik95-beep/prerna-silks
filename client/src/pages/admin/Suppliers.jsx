  ...
  return (
    <div className="admin-layout"><AdminSidebar />
      <main className="admin-main">
        <div className="admin-header"><h1 style={{display:'flex',alignItems:'center',gap:10}}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg> Suppliers</h1><button className="btn-buy" onClick={openAdd}>+ Add Supplier</button></div>
              ...
              {items.length===0 ? <tr><td colSpan={7} style={{textAlign:'center',padding:40,color:'var(--text-muted)'}}>No suppliers. Add one to start.</td></tr> :
                items.map(s=>(
                  <tr key={s.id}><td><strong>{s.name}</strong></td><td>{s.contact_person||'—'}</td><td>{s.city}</td><td>{s.phone}</td><td>{s.gst||'—'}</td>
                    ...
                    <td><button className="btn btn-outline-primary btn-sm me-1" onClick={()=>openEdit(s)}><EditIcon /></button><button className="btn btn-outline-danger btn-sm" onClick={()=>del(s.id)}><DeleteIcon /></button></td>
                  </tr>
                ))
              }
            </tbody>
